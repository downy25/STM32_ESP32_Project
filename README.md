# What to do?
- 2개의 stm32보드를 가지고 ESP32모듈을 사용하여 한쪽은 Ap mode 다른 한쪽은 Station mode로 통신하는 구조, Ap mode 쪽에선 OLed모니터에 나와있는 메뉴를 선택하면 제어 명령을 반대편 Station쪽으로 날려
Station쪽에 달려있는 모터 및 RGB LED를 제어하는 프로젝트 (각각의 모듈은 Wifi로 통신함)
